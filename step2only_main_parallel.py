...
if args.prompt1 and args.prompt2:
    ...
    if use_ui:
        notify_info("お知らせ", "翻訳と1段階目はスキップします。スクリーニングはTrueとみなしますので予めFalseは除いたファイルを用意してください")
        ...
        # UIなしでは詳細抽出プロンプト（--prompt2）だけ必須にする
        if args.prompt2:
            prompt_path_stage2 = args.prompt2
            prompt_path_stage1 = args.prompt1 if args.prompt1 else None
        else:
            _fail_and_exit("--prompt2 を指定するか、UI有効で実行してください。")
...
#入力するセルの列位置
incl=last_col + 1
ws.cell(row=1, column=incl, value="2_Judgment")
ws.cell(row=1, column=incl+1, value="2_Reason")
ws.cell(row=1, column=incl+2, value="(In Case Stage2 Error: Output of LLM)")
wb.save(wbPath2)
...
try:
    for index, row in iter_df.iterrows():
        ...
        # --- 1段階目: スクリーニング ---
        print("ステージ1: ユーザの指定によりスクリーニング処理はスキップします...")
        #result_stage1 = subprocess.run(
        #    ["python", Gemini_summarize_path, str(Abstract_text), Gemini_APIKey, prompt_path_stage1, str(Title_text), model_stage1],
        ...
        #    head = (parsed.get("_raw_text", raw_output_stage1) if isinstance(parsed, dict) else raw_output_stage1)[:160]
        #    reason_text = f"非JSON出力のため棄却（Stage1）。parse_error={err}; head='{head}'"
        #    error_text= raw_output_stage1

        is_relevant = True
        reason_text = "ユーザの指定によりスクリーニング処理はスキップされ、Trueとみなされました。"
        error_text = ""
