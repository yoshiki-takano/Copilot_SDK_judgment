...
def merge_parts_into_single(
    ...
    """
    outdir 内の Part1〜PartN の最新出力を集めて 1つのExcelに結合する。
    重要:
      - 全Partのヘッダーを union してから結合する
      - Partごとに存在しない列は空欄で埋める
      - 列順は「最初に見つかった順」を維持する
    戻り値:
      - 作成した merged xlsx のフルパス
    """
    ...
    # 例:
    # input_base_noext = "sample_Part10"
    # prefix_part     = "sample_Part"
    # -> part basename は sample_Part1, sample_Part2, ...
    def find_latest_part_file(part_no: int) -> str | None:
        target_base = f"{prefix_part}{part_no}"

        # 出力命名想定:
        #   YYYY-mm-dd_HHMMSS_<target_base>.xlsx
        # に一致するものを候補にする
        pattern = os.path.join(outdir, f"*_{target_base}.xlsx")
        candidates = glob.glob(pattern)

        # 念のため basename 完全一致でもフィルタ
        filtered = []
        for p in candidates:
            stem = os.path.splitext(os.path.basename(p))[0]
            if stem.endswith(f"_{target_base}"):
                filtered.append(p)
    ...
    def load_header_and_rows(xlsx_path: str, sheetname: str) -> tuple[list[str], list[list[Any]]]:
        wb_in = load_workbook(xlsx_path, read_only=True, data_only=True)
        try:
            if sheetname not in wb_in.sheetnames:
                raise ValueError(f"シート '{sheetname}' が見つかりません: {xlsx_path}")
    ...
    for part_no in range(1, parts + 1):
        p = find_latest_part_file(part_no)
        if p is not None:
            part_files.append((part_no, p))

    if not part_files:
        raise FileNotFoundError(
            f"結合対象ファイルが見つかりません。outdir={outdir}, input_base_noext={input_base_noext}"
        )
    ...
    loaded_parts: list[tuple[int, str, list[str], list[list[Any]]]] = []

    for part_no, path in part_files:
        headers, rows = load_header_and_rows(path, sheetname)

        # 完全空ヘッダーは除外
        normalized_headers: list[str] = []
        for h in headers:
            h2 = "" if h is None else str(h).strip()
            normalized_headers.append(h2)
            if h2 and h2 not in seen_headers:
                seen_headers.add(h2)
                union_headers.append(h2)

        loaded_parts.append((part_no, path, normalized_headers, rows))
    ...
    # prefix_part は "...Part" なので末尾 Part を外して merged 名に使う
    base_prefix = re.sub(r"Part$", "", prefix_part)
    merged_name = f"{formatted_datetime}_{base_prefix}MERGED.xlsx"
    merged_path = os.path.join(outdir, merged_name)

    # -------------------------
    # 4) write_only で merged workbook 作成
    # -------------------------
    ...
    union_index = {h: idx for idx, h in enumerate(union_headers)}

    # -------------------------
    # 5) 各Partのデータ行を union header に合わせて流し込む
    # -------------------------
    for part_no, path, headers, rows in loaded_parts:
        part_index_map: dict[int, int] = {}
        for src_idx, h in enumerate(headers):
            h2 = "" if h is None else str(h).strip()
            if h2 and h2 in union_index:
                part_index_map[src_idx] = union_index[h2]

        for row in rows:
            out_row = [""] * len(union_headers)
            for src_idx, value in enumerate(row):
                if src_idx in part_index_map:
                    dst_idx = part_index_map[src_idx]
                    out_row[dst_idx] = value
    ...
    if metadata:
        ...
        try:
            for key, value in metadata.items():
                _append_metadata_value(ws_meta, key, value)
        except Exception as e:
            ws_meta.append(["metadata_write_error", str(e)])
...
# プロンプト（TXT）
if args.prompt1 and args.prompt2:
    prompt_path_stage1 = args.prompt1
    prompt_path_stage2 = args.prompt2
else:
    if use_ui:
        notify_info("お知らせ", "翻訳処理はGoogle翻訳で行います。\n次に、1段階目（スクリーニング用）のプロンプトを選択してください。")
        ...
        notify_info("ToDo", "【2段階目】詳細抽出用のプロンプトファイル(.txt)を選択してください。")
        prompt_path_stage2 = select_file("2段階目プロンプトを選択", [("Text files", "*.txt"), ("All files", "*.*")])
        if not prompt_path_stage2:
            sys.exit("2段階目のプロンプトが選択されませんでした。終了。")
    else:
        _fail_and_exit("--prompt1 と --prompt2 を指定するか、UI有効で実行してください。")

# 実際に使うプロンプトファイルを固定表示（誤指定切り分け用）
prompt_path_stage1 = os.path.abspath(prompt_path_stage1)
prompt_path_stage2 = os.path.abspath(prompt_path_stage2)
if not os.path.exists(prompt_path_stage1):
    _fail_and_exit(f"1段階目プロンプトが見つかりません: {prompt_path_stage1}")
if not os.path.exists(prompt_path_stage2):
    _fail_and_exit(f"2段階目プロンプトが見つかりません: {prompt_path_stage2}")
print(f"[PROMPT1] {prompt_path_stage1} (sha1:{_file_sha1_short(prompt_path_stage1)})")
print(f"[PROMPT2] {prompt_path_stage2} (sha1:{_file_sha1_short(prompt_path_stage2)})")
...
try:
    for index, row in iter_df.iterrows():
        ...
        if is_relevant:
            ...
            stage2_cmd = [
                sys.executable, Gemini_summarize_path, "", Gemini_APIKey,
                prompt_path_stage2, "", "--inputs-json-file", inputs_json_file,
                "--model", args.model_stage2, "--stage", "extract"
            ]
            if args.web_search:
                stage2_cmd.append("--web-search")
            ...
            subprocess.run(
                [sys.executable, Split_Abstract_path, sum_text, wbPath2, str(row_number), str(incl+3), sheetname],
